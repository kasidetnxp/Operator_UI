...
interface UnloadLoadWorkflowProps {
  employeeId: string;
  language: Language;
  onBack: () => void;
...
export function UnloadLoadWorkflow({ employeeId, language, onBack, onTaskSubmitted }: UnloadLoadWorkflowProps) {
  ...
  // Check if destination machine has an FPC to unload
  const oldFPC = useMemo(() => {
    if (!selectedMachine) return null;
    return fullFPCList.find(fpc => fpc.location === selectedMachine) || null;
  }, [selectedMachine, fullFPCList]);

  // Dynamic validation warning/error
  const validationError = useMemo(() => {
    if (selectedMachine && !oldFPC) {
      return t.errorNoFpcOnMachine;
    }
    return '';
  }, [selectedMachine, oldFPC, t.errorNoFpcOnMachine]);
  ...
  const handleSubmitClick = () => {
    ...
    if (!oldFPC) {
      setError(t.errorNoFpcOnMachine);
      return;
    }
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      await submitUnloadLoadFPCJob(employeeId, selectedFPC!.id, selectedMachine!);
      onTaskSubmitted();
    } catch {
      setError(t.error_network);
    } finally {
